...
Write-Host "Release data prepared. Please set GITHUB_TOKEN environment variable and run:"
Write-Host "`$env:GITHUB_TOKEN = 'your_token_here'"
Write-Host "Invoke-RestMethod -Uri 'https://api.github.com/repos/$repo/releases' -Method Post -Headers @{Authorization = 'token `$env:GITHUB_TOKEN'; Accept = 'application/vnd.github.v3+json'} -Body `$releaseData -ContentType 'application/json'"

Write-Host "`n📦 To upload mycelium-app.exe after creating release:"
Write-Host "Invoke-RestMethod -Uri 'UPLOAD_URL' -Method Post -Headers @{Authorization = 'token `$env:GITHUB_TOKEN'; Accept = 'application/vnd.github.v3+json'; ContentType = 'application/octet-stream'} -Body `$fileBytes" 